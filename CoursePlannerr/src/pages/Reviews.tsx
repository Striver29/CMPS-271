  ...
  return (
    <span style={{ display: "inline-flex", gap: 6, cursor: "pointer" }}>
      {Array.from({ length: 5 }).map((_, i) => (
        <span
          key={i}
          style={{ color: i < (hovered || value) ? "var(--accent)" : "var(--border)", fontSize: 28, transition: "color 0.1s" }}
...
function RatingBar({ label, count, total }: { label: string; count: number; total: number }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 10 }}>
      <span style={{ width: 76, textAlign: "right", fontSize: 13, color: "var(--muted)" }}>{label}</span>
      ...
      <span style={{ width: 20, fontSize: 13, color: "var(--muted)", textAlign: "right" }}>{count}</span>
    </div>
  );
}
...
export default function Reviews() {
  ...
  useEffect(() => {
    if (suppressCourseSearch.current) { suppressCourseSearch.current = false; return; }
  ...
  useEffect(() => {
    if (suppressProfSearch.current) { suppressProfSearch.current = false; return; }
    if (!profApiQuery || profApiQuery.length < 2) { setProfResults([]); return; }
  ...
  const renderScorePanel = (score: string | undefined, count: number, name: string, subtitle?: string, extras?: React.ReactNode) => (
    <div style={card}>
      <div style={{ display: "flex", alignItems: "flex-end", gap: 8, marginBottom: 6 }}>
        <span style={{ fontSize: 72, fontWeight: 900, lineHeight: 1, color: "var(--text)" }}>{count ? score : "—"}</span>
        <span style={{ fontSize: 22, color: "var(--muted)", marginBottom: 12 }}>/5</span>
      </div>
      <div style={{ fontSize: 13, color: "var(--muted)", marginBottom: 20 }}>
        Overall Quality Based on <strong style={{ color: "var(--text)" }}>{count} rating{count !== 1 ? "s" : ""}</strong>
      </div>
      <div style={{ fontSize: 26, fontWeight: 800, color: "var(--text)", marginBottom: 4 }}>{name}</div>
        ...
        {!showForm && !submitted && (
          authLoading ? null : !userId ? (
            ...
            <button
              ...
              style={{ background: "var(--accent)", color: "#fff", border: "none", padding: "11px 28px", borderRadius: 30, cursor: "pointer", fontSize: 14, fontWeight: 700 }}
            >Rate →</button>
          )
        )}
  ...
  const renderDistribution = (dist: Record<number, number>, total: number) => (
    <div style={card}>
      <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 20, color: "var(--text)" }}>Rating Distribution</div>
      {[5, 4, 3, 2, 1].map(n => (
    ...
    <div style={{ ...card, marginBottom: 24 }}>
      ...
      <div style={{ display: "flex", gap: 10 }}>
        <button
          ...
          style={{ background: "var(--accent)", color: "#fff", border: "none", padding: "10px 28px", borderRadius: 30, cursor: "pointer", fontSize: 14, fontWeight: 700, opacity: formRating === 0 || submitting ? 0.6 : 1 }}
        >
          {submitting ? "Submitting..." : "Submit"}
        </button>
        <button
          onClick={() => { setShowForm(false); setSubmitError(null); }}
          style={{ background: "none", border: "1px solid var(--border)", color: "var(--muted)", padding: "10px 24px", borderRadius: 30, cursor: "pointer", fontSize: 14 }}
        >Cancel</button>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)", color: "var(--text)", fontFamily: "system-ui, sans-serif" }}>
      <header style={{ display: "flex", alignItems: "center", padding: "14px 32px", borderBottom: "1px solid var(--border)", background: "var(--panel)", gap: 20 }}>
        <button
          onClick={() => navigate("/")}
          style={{ background: "none", border: "1px solid var(--border)", color: "var(--muted)", padding: "6px 14px", borderRadius: 6, cursor: "pointer", fontSize: 13 }}
        >← Planner</button>
        ...
        <div style={{ marginLeft: "auto", display: "flex", gap: 4 }}>
          {(["courses", "professors"] as Tab[]).map(t => (
            <button
              key={t}
              onClick={() => { setTab(t); setShowForm(false); setSubmitted(false); setSubmitError(null); }}
              style={{ background: tab === t ? "var(--accent)" : "none", border: "1px solid", borderColor: tab === t ? "var(--accent)" : "var(--border)", color: tab === t ? "#fff" : "var(--muted)", padding: "7px 22px", borderRadius: 6, cursor: "pointer", fontSize: 14, textTransform: "capitalize" }}
            >{t}</button>
          ))}
        </div>
      </header>
      ...
      <div style={{ maxWidth: 900, margin: "40px auto", padding: "0 24px" }}>
        <div style={{ position: "relative", marginBottom: 32 }}>
          <input
            ...
            onChange={e => {
              if (tab === "courses") {
                setCourseSearch(e.target.value); setSelectedCourse(null); setShowForm(false);
              } else {
                setProfSearch(e.target.value); setProfApiQuery(e.target.value); setSelectedProf(null); setShowForm(false);
              }
            }}
          />
          {(tab === "courses" ? courseResults : profResults).length > 0 && (
            <div style={{ position: "absolute", top: "100%", left: 0, right: 0, background: "var(--panel)", border: "1px solid var(--border)", borderRadius: 10, zIndex: 100, marginTop: 4, overflow: "hidden" }}>
              {tab === "courses"
                ? courseResults.map(c => (
                  <div key={`${c.department}-${c.course_number}`} onClick={() => handleSelectCourse(c)}
                    style={{ padding: "12px 18px", cursor: "pointer", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 14 }}>
                    <span style={{ fontWeight: 700, color: "var(--text)", minWidth: 90 }}>{c.department} {c.course_number}</span>
                    <span style={{ fontSize: 13, color: "var(--muted)" }}>{c.title}</span>
                  </div>
                ))
                : profResults.map(p => (
                  <div key={p.id} onClick={() => handleSelectProf(p)}
                    style={{ padding: "12px 18px", cursor: "pointer", borderBottom: "1px solid var(--border)" }}>
                    <span style={{ fontWeight: 600, color: "var(--text)" }}>{formatProfName(p.full_name)}</span>
                  </div>
                ))
              }
            </div>
            ...
            <div>
                ...
                {renderScorePanel(
                  ...
                  courseAvg?.count ? (
                    <div style={{ paddingTop: 16, borderTop: "1px solid var(--border)" }}>
                      <span style={{ fontSize: 24, fontWeight: 800, color: "var(--text)" }}>{courseAvg.difficulty}</span>
                      <span style={{ fontSize: 13, color: "var(--muted)", marginLeft: 8 }}>Level of Difficulty</span>
                    </div>
                  ) : null
                )}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {courseRatings.length === 0
                  ? <div style={{ textAlign: "center", color: "var(--muted)", padding: 48, ...card }}>No reviews yet. Be the first!</div>
                  : courseRatings.map(r => (
                    <div key={r.id} style={card}>
                      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: r.review ? 12 : 0 }}>
                        <ScoreBadge score={r.rating} />
                        {r.difficulty > 0 && <span style={{ fontSize: 13, color: "var(--muted)" }}>Difficulty: {r.difficulty}/5</span>}
        ...
        {tab === "professors" && selectedProf && (() => {
          ...
          return (
            <div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24, marginBottom: 24 }}>
                {renderScorePanel(profAvg?.rating, profAvg?.count ?? 0, formatProfName(selectedProf.full_name))}
